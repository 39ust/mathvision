...
int PolygonDemo::polyArea(const std::vector<cv::Point>& vtx)
{
	...
	// area = 1/2 * abs(sum(xi+xi+1)(yi-yi+1)) 식을 이용하여 구함
	
	// (ref.) http://1nsunym.github.io/ko/online/01-09-2016//algorithm-polygon-area
	// (ref.) https://degurii.tistory.com/47
	// (ref.) https://www.mathopenref.com/coordpolygonarea2.html

	int area = 0; //Accumlates area in the roop

	int NumberPoints = (int)vtx.size(); // The last vertex is the 'previous' one to the first
	int j = NumberPoints - 1;

	for (int i = 0; i < NumberPoints; i++)
	{
		area = area + (vtx[j].x * vtx[i].x - vtx[j].y * vtx[i].y);
		j = i; // j is previous vertex to i
	}

	int abs_area = abs(area / 2);
...
/*
// Calculate the cross product between points of two shapes

int NumberPoints = (int)pts.size();

for (int i = 0; i < NumberPoints; i++)
{
	int i_minus = (i + NumberPoints - 1) % NumberPoints;
	int i_plus = (i + NumberPoints + 1) % NumberPoints;

	crossproduct[i] = (NumberPoints[i].x - NumberPoints[i_minus].x)*(NumberPoints[i_plus].y - NumberPoints[i].y)
		- (NumberPoints[i_plus].x - NumberPoints[i].x)*(NumberPoints[i].y - NumberPoints[i_minus].y);
}

// Check CrossProduct sign

if (crossprodut[0] == -1 && crossprodut[1] == -1)
	return NORMAL;
else if (crossprodut[0] == -1 && crossprodut[1] == 1)
	return REFLECTION;
else if (crossprodut[0] == -1 && crossprodut[1] == -1)
	return Concave;
